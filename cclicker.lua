...
local function drawTitle()
    ...
    print("COOKIE CLICKERS - V1")     --Title
    print(string.rep("=", w))
...
local function runGame()
  while true do -- The continuous loop of the game
    ...
    if (event == "mouse_click" and x >= 28 and 38 >= x and y >= 4 and 7 >= y) then -- PICKAXE CLICK
      if cookies >= pickaxePrice then
        cookies = cookies - pickaxePrice
        pickaxeAmount = pickaxeAmount + 1
      end
    end
    if (event == "mouse_click" and x >= 40 and 50 >= x and y >= 4 and 7 >= y) then -- VILLAGER CLICK
      if cookies >= villagerPrice then
        cookies = cookies - villagerPrice
        villagerAmount = villagerAmount + 1
      end
    end
    if (event == "mouse_click" and x >= 28 and 38 >= x and y >= 9 and 12 >= y) then -- FARM CLICK
      if cookies >= villagerPrice then
        cookies = cookies - farmPrice
        farmAmount = farmAmount + 1
      end
    end
    if (event == "mouse_click" and x >= 40 and 50 >= x and y >= 9 and 12 >= y) then -- MINE CLICK
      if cookies >= minePrice then
        cookies = cookies - minePrice
        mineAmount = mineAmount + 1
      end
    end
    if (event == "mouse_click" and x >= 28 and 38 >= x and y >= 14 and 17 >= y) then -- FACTORY CLICK
      if cookies >= factoryPrice then
        cookies = cookies - factoryPrice
        factoryAmount = factoryAmount + 1
      end
    end
    if (event == "mouse_click" and x >= 40 and 50 >= x and y >= 14  and 17 >= y) then -- PORTAL CLICK
...
term.clear()
print("Programmed by Kyle Phillips,")
